...
while true; do
  ...
  while IFS= read -r track_path; do
    ...
    # Исправленный фильтр: финальный видеопоток теперь называется [v_out]
    ffmpeg -re -loop 1 -i bg.jpg -i "$track_path" \
      -filter_complex "[1:a]afade=t=in:ss=0:d=3,afade=t=out:st=$fade_out_start:d=3,asplit[a_out][a_eq]; \
                       ...
                       [v_bg]drawtext=fontfile=/usr/share/fonts/truetype/liberation/LiberationSans-Bold.ttf:text='$display_name':x=w-mod(t*100\,w+tw):y=h-60:fontsize=32:fontcolor=white:box=1:boxcolor=black@0.6:boxborderw=10[v_out]" \
      -map "[v_out]" -map "[a_out]" \
