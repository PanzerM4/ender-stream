#!/bin/bash
set -e

CD_DIR="/radio"
cd "$CD_DIR"

# Остановка старых процессов
pkill -9 -f "ffmpeg" || true
pkill -9 -f "http.server" || true

# Проверка файлов
if ! ls *.mp3 >/dev/null 2>&1; then
  echo "Нет mp3-файлов в /radio"
  exit 1
fi
if [ ! -f bg.jpg ]; then
  echo "Нет bg.jpg в /radio"
  exit 1
fi

# Заглушка для Render (берет порт из переменной окружения или 10000)
PORT=${PORT:-10000}
python3 -m http.server "$PORT" >/dev/null 2>&1 &
HTTP_PID=$!
trap "kill $HTTP_PID 2>/dev/null; rm -f playlist.fifo" EXIT

echo "Запуск радио с $(ls *.mp3 | wc -l) треками"

# Функция генерации плейлиста (случайный порядок, формат concat demuxer)
generate_playlist() {
  for f in *.mp3; do
    echo "file '$(pwd)/$f'"
  done | shuf   # если нет shuf, замени на sort -R
}

# Вечный цикл: пересоздаём плейлист, запускаем ffmpeg
while true; do
  rm -f playlist.fifo
  mkfifo playlist.fifo

  # Пишем плейлист в FIFO и закрываем, чтобы ffmpeg начал играть
  generate_playlist > playlist.fifo &
  FEEDER_PID=$!

  ffmpeg -v error -nostdin -y \
    -loop 1 -r 5 -i bg.jpg \
    -re -f concat -safe 0 -i playlist.fifo \
    -filter_complex \
      "[1:a]asplit[audio_out][audio_vis]; \
       [audio_vis]showwaves=s=1280x80:mode=cline:colors=white@0.6:r=5[waves]; \
       [0:v]scale=1280:720[bg]; \
       [bg][waves]overlay=x=0:y=H-h[v_waves]; \
       [v_waves]drawtext=text='RADIO LIVE':x=30:y=h-130:fontsize=24:fontcolor=white:box=1:boxcolor=black@0.5:boxborderw=10:fontfile=/usr/share/fonts/truetype/liberation/LiberationSans-Bold.ttf[video_out]" \
    -map "[video_out]" -map "[audio_out]" \
    -c:v libx264 -preset ultrafast -tune stillimage -crf 30 -b:v 800k -maxrate 800k -bufsize 1600k \
    -pix_fmt yuv420p -g 10 -c:a aac -b:a 128k -ar 44100 \
    -f flv "rtmp://a.rtmp.youtube.com/live2/4ux7-0ay8-816w-cxrb-1j24"

  kill $FEEDER_PID 2>/dev/null || true
  rm -f playlist.fifo
  echo "FFmpeg завершился, перезапуск через 5 секунд..."
  sleep 5
done
